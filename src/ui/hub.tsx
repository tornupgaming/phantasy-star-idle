 ...
 * over the persistent scene layer — status cluster, money pod, nav window,
 * the active pane's window(s) + detail window, and the shopkeeper dialogue
 * window along the bottom. Keyboard navigation (ui-navigation) lives here as
 * one document-level listener whose focus state is the kbdMenu signal.
 */

import { For, Match, Show, Switch, createEffect, onCleanup, onMount } from "solid-js";
import { CLASS_BY_ID, LEVEL_CAP } from "../engine/classes";
import { xpForLevel } from "../engine/progression";
import { useUi } from "./context";
import { Icon, SpriteDefs, WindowBox } from "./components";
...
function StatusCluster() {
  ...
  return (
    <div class="pso-window status-cluster">
      ...
      <div class="status-main">
        ...
        <div class="muted small">
          {CLASS_BY_ID[c().classId].name} · {c().sectionId}
        </div>
        <div class="xp-bar">
...
function MoneyPod() {
  const ui = useUi();
  return (
    <div class="pso-window money-pod">
      <span class="meseta">
