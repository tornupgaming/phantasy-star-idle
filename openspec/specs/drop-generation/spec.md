# drop-generation Specification

## Purpose
Procedurally generate authentic PSO drops — per-kill and per-box item generation from the resolved Blue Burst tables, keyed by section ID, difficulty, and area, with rare-spec checks, generated item variance (grind, bonuses, special, slots, stars), and full seeded-RNG determinism.
