# area-catalog Specification

## Purpose
Define the catalog of selectable run areas — one per authentic PSO Episode 1 floor, grouped by zone and episode — including standalone single-boss arenas, per-area recommended ATP, stage-generation coverage of the full Ep1 spawn-type set (gadget skip list, rare-variant pairs), and legacy-id compatibility for persisted saves.
