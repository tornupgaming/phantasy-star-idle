# shop-table-data Specification

## Purpose
Reproducibly extract newserv's BB shop random-set tables (armor/tool shops, per-difficulty weapon shops) and their code-resident mapping constants into a checked-in engine dataset (mirrors the drop-table-data pipeline pattern).
