# shop-list-card Specification

## Purpose
Render shop stock lists (weapon counter, armour counter, tool shop) as Nova-style cards: layered glass finish, numbered slot tabs, recessed icon wells, per-kind stat chips, and warm amber selection on a cool cyan base, while preserving the existing selection/detail-pane semantics. (TBD: refine as the capability evolves.)
