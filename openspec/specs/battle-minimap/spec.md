# battle-minimap Specification

## Purpose
Display a spatial minimap of the current floor during a run: one cell per authentic room positioned by the rolled layout's geometry, with room states (current/cleared/unvisited) derived purely from revealed events so the map never leaks the run's outcome. (TBD: refine as the capability evolves.)
