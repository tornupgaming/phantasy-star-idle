# reactive-ui-architecture Specification

## Purpose
Define the reactive UI architecture: fine-grained menu rendering, transient UI state held outside the DOM, imperative islands for the battle stage and hub backdrop, and a one-way engine-state bridge that keeps the simulation engine framework-agnostic. (TBD: refine as the capability evolves.)
