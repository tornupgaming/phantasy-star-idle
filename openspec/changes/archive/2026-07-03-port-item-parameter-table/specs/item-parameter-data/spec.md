...
Every weapon definition SHALL carry its authentic animation category (`WeaponKind`, 19 values: fist, saber, sword, dagger, partisan, slicer, handgun, rifle, mechgun, shot, cane, rod, wand, claw, double-saber, twin-sword, katana, launcher, card). The pacing module SHALL map every weapon kind onto one of the existing five attack-speed archetypes via a single total lookup table, leaving existing archetype timings unchanged. Authentic per-class frame data is explicitly deferred; the lookup table is the designated replacement seam.

#### Scenario: Every weapon kind resolves to an archetype

- **WHEN** any weapon definition in the dataset is queried for its combo-step timing
- **THEN** its `WeaponKind` resolves through the lookup table to one of the five existing archetypes and existing curated weapons' timings are unchanged
